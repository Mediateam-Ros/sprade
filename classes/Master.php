...
Class Master extends DBConnection {
	...
	function save_teamvergleich(){
		...
		$sql = "UPDATE `teamvergleich` set {$data} ";
		$save =  $this->conn->query($sql);
		$this->capture_err();
		// return json_encode($resp);
	}
	...
	function get_account(){
		extract($_POST);
		$qry = $this->conn->query("SELECT id,concat(lastname,', ',firstname) as name FROM `accounts` where account_number = '{$account_number}' ");
		$this->capture_err();
		if($qry->num_rows > 0){
			$resp['status'] = 'success';
...
switch ($action) {
	...
	break;
	case 'save_teamvergleich':
		echo $Master->save_teamvergleich();
	break;
	// case 'withdraw':
